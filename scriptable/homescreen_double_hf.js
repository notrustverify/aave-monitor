...
class AAVEHealthMonitor {
  ...
  async run() {
    try {
      ...
      const addresses = indices.map(idx => USER_CONFIG.addresses[idx]).filter(Boolean);
      if (addresses.length === 0) throw new Error('No address selected');
      
      // Parallel fetch data for all addresses
      const dataList = await Promise.all(addresses.map(addr => this.fetchAAVEData(addr)));
      
      // Shows both addresses in the widget
      await this.createMultiWidget(dataList, addresses);
      
      // Refresh based on HF
      const minHealth = Math.min(...dataList.map(d => (typeof d.healthFactor === 'number' ? d.healthFactor : 100)));
 ...
 async renderSingleAddress(stack, data, addressConfig) {
  ...
  async fetchAAVEData(addressConfig) {
    const network = NETWORKS[addressConfig.network];
    if (!network) throw new Error(`Unsupported network: ${addressConfig.network}`);
