...
// AAVE Monitor - Scriptable Widget - Home Screen One Account Health Factor
...
const USER_CONFIG = {
  ...
  displayMode: 'healthfactor',  // Options: 'healthfactor'
...
const NETWORKS = {
  ...
  base: {
    ...
    logoUrl: 'https://github.com/base-org/brand-kit/raw/main/logo/in-product/Base_Network_Logo.png',
    chainId: 8453
  },
  gnosis: {
    name: 'Gnosis',
  ...
  bsc: {
    ...
    logoUrl: 'https://raw.githubusercontent.com/trustwallet/assets/master/blockchains/binance/assets/BUSD-BD1/logo.png',
    chainId: 56
  }
...
class AAVEHealthMonitor {
  ...
  async run() {
    try {
      ...
      const primaryAddress = USER_CONFIG.addresses[USER_CONFIG.primaryAddressIndex];
      if (!primaryAddress) {
      ...
      this.setSmartRefreshInterval(aaveData.healthFactor, primaryAddress.network);
  ...
  setSmartRefreshInterval(healthFactor, network) {
    ...
    if (healthFactor === '∞' || healthFactor === Infinity || healthFactor > USER_CONFIG.warningThreshold) {
    ...
    const finalInterval = Math.max(1, Math.round(baseInterval * networkMultiplier));
    ...
    console.log(`Smart refresh: ${finalInterval} minutes (base: ${baseInterval}, network: ${network}, multiplier: ${networkMultiplier})`);
  }
  ...
  async createHealthFactorWidget(data, addressConfig) {
    ...
    if (USER_CONFIG.showLogo && network.logoUrl) {
      try {
        ...
        if (logo) {
          ...
          logoImage.imageSize = new Size(USER_CONFIG.logoSize, USER_CONFIG.logoSize);
        }
      } catch (error) {
    ...
    // Health factor value
    const healthText = valueStack.addText(data.healthFactor.toFixed(2).toString());
    ...
    // Last updated
    const updateText = this.widget.addText(`${new Date().toLocaleTimeString()}`);
  ...
  async createFullWidget(data, addressConfig) {
    ...
    if (settings.showNetworkName) {
      const networkText = infoStack.addText(network.name);
      networkText.font = Font.mediumSystemFont(12);
      networkText.textColor = this.textColor;
    }
    
    if (settings.showAddressLabel && addressConfig.label) {
      const labelText = infoStack.addText(addressConfig.label);
      labelText.font = Font.systemFont(10);
      labelText.textColor = this.grayColor;
    }
    ...
    const healthLabel = healthStack.addText('Health Factor: ');
    healthLabel.font = Font.mediumSystemFont(16);
    healthLabel.textColor = this.textColor;
    
    const healthValue = healthStack.addText(data.healthFactor.toFixed(2).toString());
    healthValue.font = Font.boldSystemFont(18);
    ...
    if (settings.showCollateral) {
      this.addMetricRow('Collateral', `$${this.formatNumber(data.totalCollateralUSD)}`);
    }
    ...
    if (settings.showAvailableBorrows) {
      this.addMetricRow('Available', `$${this.formatNumber(data.availableBorrowsUSD)}`);
    }
    ...
    if (settings.showLiquidationThreshold) {
      this.addMetricRow('Liq. Threshold', `${(data.liquidationThreshold * 100).toFixed(1)}%`);
    }
    
    // Last updated
    this.widget.addSpacer(4);
    const updateText = this.widget.addText(`Updated: ${new Date().toLocaleTimeString()}`);
    updateText.font = Font.systemFont(8);
    updateText.textColor = this.grayColor;
  }
  ...
  getHealthFactorColor(healthFactor) {
    if (healthFactor === '∞' || healthFactor === Infinity || healthFactor > 10) {
  ...
  parseResponse(result, addressConfig, network) {
    ...
    const currentLiquidationThreshold = BigInt('0x' + cleanResult.substr(192, 64));
  ...
  async getNetworkLogo(network) {
    ...
    try {
      ...
      const logoFile = fm.joinPath(cacheDir, network.name.toLowerCase() + '.png');
